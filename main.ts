...
export default class TodoistStyleTasksPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerMarkdownPostProcessor((el: HTMLElement, _ctx: MarkdownPostProcessorContext) => {
      ...
      console.log("[checklists] found checkboxes:", checkboxes.length);
      ...
      checkboxes.forEach((cb) => {
        ...
        const update = () => {
          ...
          if (shouldHide) console.log("[checklists] hid <li>", li.textContent?.trim());
        };
    ...
    this.addCommand({
      id: "reveal-completed-5s",
      name: "Reveal Completed Tasks (5 seconds)",
      callback: () => {
        document.body.classList.add("todoist-reveal-completed");
        this.refreshAllEditors();
        this.applyReadingViewHidingNow();

        if (this.revealTimer) window.clearTimeout(this.revealTimer);
        this.revealTimer = window.setTimeout(() => {
          document.body.classList.remove("todoist-reveal-completed");
          this.refreshAllEditors();
          this.applyReadingViewHidingNow();
  ...
  onunload() {
    ...
    if (this.revealTimer) window.clearTimeout(this.revealTimer);
    document.body.classList.remove("todoist-hide-completed", "todoist-reveal-completed");
  }
  ...
  private applyBodyClasses() {
    ...
    console.log("[checklists] body.classList ->", document.body.classList.value);
  }
  ...
  private applyReadingViewHidingNow() {
    ...
    const reveal = document.body.classList.contains("todoist-reveal-completed");
  ...
  private makeEditorHider() {
    const plugin = this;
    const hideLineDeco = Decoration.line({ attributes: { class: "todoist-hide-line" } });
    ...
    function buildDecorations(state: EditorState): DecorationSet {
      ...
      if (!(plugin.settings.hideCompleted && plugin.settings.hideInEditor && cssOn)) {
